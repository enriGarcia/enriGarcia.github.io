...
#### <u>Adapting H-In.nity Controller for the Desired Reference Tracking of the Sphere Position in the Maglev Process</u>
*The Maglev process consists in a magnetic .eld generated by the modulation of a coil current which yields the sphere levitation. Foucault current is the current trans- mitted from the coil to a sphere in the Maglev process which yields the undesired sphere vibrations. In this study, an adapting H-in.nity controller is introduced as the combination of the adapting and H-in.nity strategies for the desired reference tracking of the sphere position in the Maglev process. The adapting strategy is used for the unknown dynamics estimation, and the H-in.nity strategy is used for the desired reference tracking. The Lyapunov technique is used to satisfy the error convergence and the H-in.nity criterion. Two simulations show the e¤ectiveness of the considered method for the desired reference tracking of the sphere position in the Maglev process.*
